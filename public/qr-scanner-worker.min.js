...
(function () {
  ...
  function X(a, b) {
    ...
    for (g = 0; g < e; g++) {
      h = a.inverse(b[g]);
      let c = 1;
      for (let d = 0; d < e; d++) g !== d && (c = a.multiply(c, J(1, a.multiply(b[d], h))));
  ...
  function ea(a, b, c) {
    ...
    if (1 < d.ecBlocks.length)
      for (g = d.ecBlocks[0].numBlocks, b = d.ecBlocks[1].numBlocks, c = 0; c < b; c++)
  ...
  function ja(a, b, c, d) {
    d = (u(z(a, c, d, 5)) / 7 + u(z(a, b, d, 5)) / 7 + u(z(c, a, d, 5)) / 7 + u(z(b, a, d, 5)) / 7) / 4;
  ...
  function N(a, b, c, d) {
    ...
    for (let n = g, v = h; n !== a + p; n += p) {
      g = f ? v : n;
      h = f ? n : v;
      if (c.get(g, h) !== r && ((r = !r), e.push({ x: g, y: h }), e.length === d + 1)) break;
      q += l;
      if (0 < q) {
        if (v === b) break;
        v += m;
    ...
    c = [];
    for (f = 0; f < d; f++) e[f] && e[f + 1] ? c.push(x(e[f], e[f + 1])) : c.push(0);
    return c;
  }
  function z(a, b, c, d) {
    let e = b.y - a.y,
  ...
  function O(a, b, c) {
    try {
      ...
      return (
        Math.sqrt(h.error * h.error + k.error * k.error + l.error * l.error + q.error * q.error) +
  ...
  function ka(a) {
      ...
      for (let b = -1; b <= a.width; b++) {
        ...
        if (g === l) f++;
          ...
          h =
            ...
            !g;
          var k = u(n.slice(-3)) / 3;
          g = Math.abs(n[2] - k) < k && Math.abs(n[3] - k) < k && Math.abs(n[4] - k) < k && g;
          if (h) {
            let a = b - n[3] - n[4],
              d = a - n[2];
    ...
    b = b
      ...
      .map((b) => {
        ...
        if (a.get(Math.round(c), Math.round(d)))
          return (
            (b = [b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1]),
            (b = u(b) / b.length),
      ...
      .map((a, b, c) => {
        ...
        c = c
          .filter((a, c) => b !== c)
          .map((b) => ({ x: b.x, y: b.y, score: b.score + Math.pow(b.size - a.size, 2) / a.size, size: b.size }))
          .sort((a, b) => a.score - b.score);
    ...
    let { topRight: l, topLeft: q, bottomLeft: p } = ia(b[0].points[0], b[0].points[1], b[0].points[2]);
    c = P(a, d, l, q, p);
    b = [];
    c &&
      b.push({
    ...
    b = b
      .map((b) => {
        ...
        if (a.get(Math.floor(c), Math.floor(d)))
          return (
            u([b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1]),
            (b = O({ x: Math.floor(c), y: Math.floor(d) }, [1, 1, 1], a) + x({ x: c, y: d }, l)),
            { x: c, y: d, score: b }
          );
      })
      .filter((a) => !!a)
        ...
        if (d) c = d;
          ...
          for (d = 0; d < c.width; d++)
            for (let a = d + 1; a < c.height; a++)
              c.get(d, a) !== c.get(a, d) && (c.set(d, a, !c.get(d, a)), c.set(a, d, !c.get(a, d)));
          c = L(c);
        }
      }
      if (c)
  ...
  function I(a, b, c, d = {}) {
    ...
    if (g.useIntegerApproximation)
      for (var p = 0; p < c; p++)
        for (var m = 0; m < b; m++) {
          var r = 4 * (p * b + m);
          q.set(m, p, (g.red * a[r] + g.green * a[r + 1] + g.blue * a[r + 2] + 128) >> 8);
        }
    else
      for (p = 0; p < c; p++)
        for (m = 0; m < b; m++)
          (r = 4 * (p * b + m)), q.set(m, p, g.red * a[r] + g.green * a[r + 1] + g.blue * a[r + 2]);
    g = Math.ceil(b / 8);
    p = Math.ceil(c / 8);
    m = g * p;
    if (h) {
    ...
    for (m = 0; m < p; m++)
      for (r = 0; r < g; r++) {
        ...
        24 >= t - v &&
          ...
          0 < m &&
            0 < r &&
            ((t = (n.get(r, m - 1) + 2 * n.get(r - 1, m) + n.get(r - 1, m - 1)) / 4), v < t && (u = t)));
        n.set(r, m, u);
      }
    h ? ((m = new Uint8ClampedArray(a.buffer, l, k)), (l += k), (m = new A(m, b))) : (m = A.createEmpty(b, c));
    r = null;
    f && (h ? ((a = new Uint8ClampedArray(a.buffer, l, k)), (r = new A(a, b))) : (r = A.createEmpty(b, c)));
    for (b = 0; b < p; b++)
      for (a = 0; a < g; a++) {
        c = g - 3;
        c = 2 > a ? 2 : a > c ? c : a;
        ...
        h = 2 > b ? 2 : b > h ? h : b;
        k = 0;
        for (l = -2; 2 >= l; l++) for (v = -2; 2 >= v; v++) k += n.get(c + l, h + v);
        c = k / 25;
        for (h = 0; 8 > h; h++)
          for (k = 0; 8 > k; k++)
            (l = 8 * a + h), (v = 8 * b + k), (t = q.get(l, v)), m.set(l, v, t <= c), f && r.set(l, v, !(t <= c));
      }
    f = f ? { binarized: m, inverted: r } : { binarized: m };
    let { binarized: w, inverted: x } = f;
    (f = Q(d ? x : w)) ||
      ("attemptBoth" !== e.inversionAttempts && "invertFirst" !== e.inversionAttempts) ||
      (f = Q(d ? w : x));
    return f;
  }
  class A {
  ...
  class V {
    ...
    readBits(a) {
      ...
      if (0 < a) {
        for (; 8 <= a; ) (b = (b << 8) | (this.bytes[this.byteOffset] & 255)), this.byteOffset++, (a -= 8);
        0 < a &&
          ((c = 8 - a),
          (b = (b << a) | ((this.bytes[this.byteOffset] & ((255 >> c) << c)) >> c)),
          (this.bitOffset += a));
      }
      return b;
    }
    ...
    constructor(a, b) {
      ...
      if (1 < c && 0 === b[0]) {
        ...
        if (d === c) this.coefficients = a.zero.coefficients;
          ...
          for (this.coefficients = new Uint8ClampedArray(c - d), a = 0; a < this.coefficients.length; a++)
  ...
  let K = [
    ...
    la = {
      ...
      greyScaleWeights: { red: 0.2126, green: 0.7152, blue: 0.0722, useIntegerApproximation: !1 },
      canOverwriteImage: !0,
    };
  I.default = I;
  let G = "dontInvert",
