    ...
    watchVideoStreamPlayback: (
      ...
      return VideoStreamPlayback.findOne({ _id }).then((videoStreamPlayback) => {
        let dPlayTime = seconds_watched - videoStreamPlayback.seconds_watched,
          dRealTime = (Date.now() - new Date(self.videoStreamPlayback.updated_at).getTime()) / 1000;
        const MOE = 2.25;
        if (seconds_watched > self.videoPlayback.seconds_watched) {
          if (dPlayTime / dRealTime > MOE) {
            throw new Error("Too fast");
          } else {
            return VideoStreamPlayback.findOneAndUpdate(videoStreamPlayback, { seconds_watched }, { new: true }).then(
              (updated) => {
                return updated;
              }
            );
